<?php
		...
		$Action_id 			 != '')
	{
		$date 			= date_create();
		$file_name 		= guid();
		$target_dir 	= $g_live_dir;
		create_folder($target_dir);
		$target_file 	= $target_dir.$file_name;// . "." . $imageFileType;
		$target_file1 	= $target_dir.$file_name."_1";// . $imageFileType;
		
		$base64_f2 = null;
		$data2 = get_image_content($Insurance_no, $Remote_insurance_no, $Person_id, $base64image, $target_file, $target_file1);
		if ($data2 != null)
		{
			...
			if ($result = mysqli_query($link, $sql))
			{
				if (mysqli_num_rows($result) > 0)
				{
					...
					*/
						...
						{
							$data = result_message("false", "0x0206", "Call API core無回應，無法比對!", "");
						...
						if ($Action == "OK")
						{
							$data = result_message("true", "0x0200", "動作相符!", "");
							$status_code = $status_code_succeed;
						}
						else if ($Action == "Fail")
						{
							$data = result_message("false", "0x0201", "動作不相符!", "");
							$status_code = $status_code_failure;
						}
						/* 2021/08/03 }else{
							//攻擊人臉
				...
				{
					$data = result_message("false", "0x0204", "身分證資料不存在!", "");
					$status_code = $status_code_failure;
				}
			}
			else
